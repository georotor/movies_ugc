...
class VerticaInsertTester(AbstractInsertTester):
    @staticmethod
...
class VerticaSelectTester(AbstractSelectTester):
    def _users_id(self):
        """Получаем список пользователей. Используется в других запросах."""
        res = self.db.select('SELECT DISTINCT user_id from test.views')
        return [user[0] for user in res]

    def _movies_id(self):
        """Получаем список фильмов. Используется в других запросах."""
        res = self.db.select('SELECT DISTINCT movie_id from test.views')
        return [user[0] for user in res]
